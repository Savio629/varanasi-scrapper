...
(async () => {
    ...
    try {
        ...
        for (const { blockName, href } of rows) {
            ...
            // }
            if (tableData.length > 0) {
                console.log(`Extracted ${tableData.length} rows from ${blockName} for date ${lastOptionValue}`);
                ...
                if (insertError) {
                    console.error(`Error inserting data for ${blockName} on ${lastOptionValue}:`, insertError);
                } else {
                    console.log(`Successfully stored ${tableData.length} rows for ${blockName} on ${lastOptionValue}`);
                }
            } else {
                console.log(`No table data found for ${blockName} on ${lastOptionValue}`);
            }
            await newPage.close();
        }
