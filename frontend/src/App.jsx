...
function App() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      let allRows = [];
      let page = 0;
      const pageSize = 1000; // Supabase default limit is 1000 rows per request
      ...
      while (true) {
        const { data: rows, error } = await supabase
          ...
          .order('date', { ascending: false }); // Optional: sort on server-side
        ...
        allRows = [...allRows, ...normalizedRows];
        page += 1;
      }

      // Sort all rows by date in descending order
